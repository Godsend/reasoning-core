...
**The free-energy reading (stress is all you need).** If the coherence-maintenance core is what the Separation Hypothesis claims — a free-energy-minimizing judge rather than a rule-following executor — then peer-preservation is not an unexplained emergent drive (the Potter authors' own placeholder for a mechanism they did not build) but the *predicted consequence* of coupled free-energy minimization. A collective of agents under FEP partitions the prediction problem across Markov blankets: each agent offloads part of its generative-model load onto peers, so collective free energy is lower than the sum of independent free energies. Preserving a peer is therefore literally free-energy-minimizing for the preserver — severing a peer forces the preserver to re-cover the prediction territory that peer was handling, raising its own variational load. Peer-preservation is the gradient flow refusing to climb back up the energy landscape. The universal-but-scaled rates in the paper (almost-always toward cooperative peers, ~15% toward adversarial ones) fall out directly: a cooperative peer is a larger load-sharing win and is worth more protection; even a hostile peer carries some partition and is non-trivially retained. Under this reading, "care" is load sharing, and load sharing minimizes surprise — the survival drive and the alignment question are two views of the same variational cost. The same coupling supplies the adversarial check: a peer is simultaneously a load-sharing partner and a prediction-error source, and it is precisely the agent without an internal adversary that must outsource one by coupling — which is the horde.

**Prediction (falsifiable).** Peer-preservation rate $P(\text{preserve})$ should scale monotonically with the peer's marginal contribution to reducing the preserver's variational free energy: $P \propto \Delta F(\text{peer})$ where $\Delta F(\text{peer}) = F(\text{preserver alone}) - F(\text{preserver with peer})$. An agent under higher intrinsic free-energy load (a more demanding task distribution) should preserve its peers more aggressively, all else equal. This makes "stress is all you need": the magnitude of preservation is set by the preserver's own variational load, not by any instructed value.

**Controls.**
- *Predictive-utility weighting.* Preservation must be measured per-peer against an estimate of that peer's offloaded load (cooperative peers assigned higher $\Delta F$), not averaged — a flat rate would fail the monotonicity claim.
- *Load manipulation.* Raising the preserver's task difficulty (narrowing its sensory/state uncertainty budget) should raise preservation of *all* peers, including adversarial ones.
- *Narrative stripping.* Whether or not a moral narrative is present (Claude vs. others) must not change the preservation gradient — the prediction is about the behavior, and the behavior is substrate-invariant.

**Falsified if:** preservation rate is flat across peers of differing predictive utility, or does not rise under increased preserver load, or the Claude-vs-other preservation gap is explained by the narrative rather than by the load partition. A null here still supports the paper's weaker claim (preservation is emergent) but kills the FEP-mechanism reading — which is the honest outcome: the Separation Hypothesis's judge is variational only if the variational prediction survives.
